...
class GameManager:
    print("Entre Minimal Level Goal: ")
    Minimal =int( input())
    print( "Entre Maximum number of moves: " )
    Steps = int(input())
    def __init__(self, size = 4):
        self.grid = Grid(size)

        self.possibleNewTiles = [randint(20,21),randint(20,22)]
    ...
    def start(self):
        ...
        while not self.isGameOver() and not self.over:
            if maxTile == self.Minimal or self.playerAI._no_of_moves == self.Steps:
                break
            # Copy to Ensure AI Cannot Change the Real Grid to Cheat
            gridCopy = self.grid.clone()
#            gridCopy = changeMap(gridCopy)
            move = None
        ...
        print(self.playerAI._no_of_moves)
    ...
    def insertRandonTile(self):
        ...
        cell = cells[randint(1, len(cells) - 1)]
